...
当前版本是一个 Windows WPF 桌面陪伴原型：桌面上有一只透明背景的占位小猫，
已接入 6 个动作、观察记录、本地持久化、轻量习惯偏好、学习反馈、安静模式、
试玩文档和可重复打包流程。
...
| M4 试玩准备 | 本地完成 | 6 个动作 ID 接入、试玩说明、反馈表、发布检查清单、打包脚本。 |
...
| `idle_sit` | 坐着发呆并眨眼。 |
| `rest_sleep` | 趴睡和呼吸起伏。 |
| `walk_slow` | 在桌面工作区内慢慢走动。 |
| `wake_stretch` | 从休息离开前先醒来伸懒腰。 |
| `edge_stop` | 到桌面边缘后停下看一眼。 |
| `pet_react` | 点击优先回应。 |

当前视觉仍是占位绘制代码与帧元数据，不是最终动画资产。
...
- 占位猫已经足够验证系统闭环，但“可爱”和“有陪伴感”仍需要更好的视觉资产检验。
