# My Cat V0.6 Playtest Release Checklist

1. Confirm the app About dialog, README, PLAYTEST, guide, feedback form, and release checklist say `V0.6 Playtest`.
...
7. Confirm the ZIP is `artifacts\playtest\MyCat-v0.6-playtest-win-x64.zip`.
...
12. Confirm `cats\my-cat\manifest.json`, all runtime `frame_*.png` files, and `toys\yarn_bell.png` are present in the extracted package.
...
21. Play one chase game, let the cat catch the yarn bell, and confirm the catch pose reads clearly.
22. Start another chase game and right-click once to confirm it stops with the miss pose.
23. Leave one real desktop run open for 30 minutes before sharing broadly.
24. Create GitHub Pre-release `My Cat 可试玩预发布版` and upload the ZIP.
